...
class ConnectorTrainer(QMainWindow):
    ...
    def save_selection(self):
        ...
        # Collect pin data and identify connections
        pins = []
        connections = []
        for row_idx in range(start_row, end_row + 1):
            row_pins = []
            for col_idx in range(start_col, end_col + 1):
                pin = str(self.df.iat[row_idx, col_idx])
                pins.append(pin)
                row_pins.append(pin)
            if len(row_pins) > 1:
                for i in range(len(row_pins) - 1):
                    connections.append((row_pins[i], row_pins[i + 1]))
    ...
    def train_model(self):
        ...
        for selection in self.selections:
            connections.extend(selection['connections'])
            for pin in selection['pins']:
                # Find common connector-pin patterns
                matches = re.findall(r'\b([A-Z]+)(\d+)\b', pin)
                for conn, pin_num in matches:
                    pattern = f"{conn}-{pin_num}"
        ...
        df_connections = pd.DataFrame(connections, columns=['Source', 'Destination'])
        
        save_path, _ = QFileDialog.getSaveFileName(self, "Save Training Data", "", "Excel Files (*.xlsx)")
        if save_path:
            with pd.ExcelWriter(save_path) as writer:
                df_training.to_excel(writer, sheet_name='Patterns', index=False)
