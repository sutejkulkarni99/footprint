...
def create_ipc7351_footprint(filename, footprint_name, num_pins, pitch, pin_diameter, border_width, border_height, tolerance=0.2, annular_ring=0.15):
    """
    ...
    # Calculate the total width and height of the footprint
    total_width = (num_pins - 1) * pitch + pad_size
    ...
    content += f"  (fp_text reference J1 (at 0 {-courtyard_height/2 - 2}) (layer F.SilkS)\n"
    ...
    for i in range(num_pins):
        x = i * pitch - (num_pins - 1) * pitch / 2  # Center the pins horizontally
        y = 0
...
class PinCalibrationApp(QMainWindow):
    ...
    def generate_footprint(self):
        ...
        # Calculate the pitch in mm
        pitch_pixels = np.sqrt((self.pin_rows[0][1][0] - self.pin_rows[0][0][0])**2 +
                               (self.pin_rows[0][1][1] - self.pin_rows[0][0][1])**2)
        ...
        if len(self.lasso_points) > 2:
            ...
            border_width = max(x for x, y in transformed_border_points) - min(x for x, y in transformed_border_points)
            border_height = max(y for x, y in transformed_border_points) - min(y for x, y in transformed_border_points)
        else:
            # Use default border dimensions if no lasso points are defined
            border_width = (max(pin[0] for row in self.pin_rows for pin in row) -
                            min(pin[0] for row in self.pin_rows for pin in row)) / self.pixel_to_mm_ratio
            border_height = (max(pin[1] for row in self.pin_rows for pin in row) -
                             min(pin[1] for row in self.pin_rows for pin in row)) / self.pixel_to_mm_ratio
        ...
        try:
            create_ipc7351_footprint(
                ...
                num_pins=len(self.pin_rows[0]),  # Number of pins in the first row
                pitch=pitch_mm,  # Pitch in mm
