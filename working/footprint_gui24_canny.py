...
class PinCalibrationApp(QMainWindow):
    ...
    def generate_2d_drawing(self):
        ...
        if len(self.lasso_points) > 2:
            # Convert polygon points to millimeters
            polygon_points_mm = [(x / self.pixel_to_mm_ratio, y / self.pixel_to_mm_ratio) for x, y in self.lasso_points]
            polygon = Polygon(polygon_points_mm, edgecolor='y', facecolor='none', linestyle='--')
            ax.add_patch(polygon)

            # Add dimensions for the polygon border
            for i in range(len(polygon_points_mm)):
                x1, y1 = polygon_points_mm[i]
                x2, y2 = polygon_points_mm[(i + 1) % len(polygon_points_mm)]
