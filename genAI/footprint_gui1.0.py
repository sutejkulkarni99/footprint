# GUI for Pin Detection with Faster R-CNN
...
def load_faster_rcnn_model():
    ...
    # model.load_state_dict(torch.load("faster_rcnn_pin_detection.pth"))
    model.eval()
    return model

# Step 2: Define the function to detect the ruler and calculate the pixel-to-millimeter ratio
def detect_ruler_and_calibrate(image):
    ...
    if contours:
        ...
        cv2.putText(image, "Ruler", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 0, 0), 2)
        ruler_length_pixels = w
        ruler_length_mm = 100  # Replace with the actual length of the ruler in millimeters
        pixel_to_mm_ratio = ruler_length_mm / ruler_length_pixels
        return pixel_to_mm_ratio, image
    else:
        print("Error: Could not detect the ruler.")
        return None, image

# Step 3: Define the function to process a single image
def process_image(image_path, model):
    ...
    # Calibrate using the ruler
    pixel_to_mm_ratio, calibrated_image = detect_ruler_and_calibrate(image)
    ...
    for box in pin_boxes:
        ...
        cv2.rectangle(calibrated_image, (x1, y1), (x2, y2), (0, 255, 0), 2)
        cv2.putText(calibrated_image, "Pin", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)

    return calibrated_image, pin_pitch_mm

# Step 4: Define the GUI
class PinDetectionApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Pin Detection with Faster R-CNN")
        self.image_paths = []
        ...
        # Create GUI elements
        self.label = tk.Label(root, text="Select images of the connector with a ruler:")
    ...
    def process_images(self):
        ...
        for i, image_path in enumerate(self.image_paths):
            processed_image, pin_pitch_mm = process_image(image_path, self.model)
