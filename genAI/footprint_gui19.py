...
from PySide6.QtWidgets import (
    ...
    QProgressBar, QMessageBox, QTextEdit, QScrollArea, QGridLayout, QGraphicsView, QGraphicsScene, QGraphicsEllipseItem, QGraphicsLineItem
)
from PySide6.QtGui import QImage, QPixmap, QTextCursor, QPainter, QColor, QMouseEvent
from PySide6.QtCore import Qt, Signal, QPointF

class DraggablePoint(QGraphicsEllipseItem):
    def __init__(self, x, y, radius, index, scene, parent=None):  # Add scene as argument
        ...
        self.index = index
        self.radius = radius
        self.scene_ref = scene #Store scene reference

    def mouseMoveEvent(self, event):
        super().mouseMoveEvent(event)
        self.scene_ref.update_lines() #update lines while dragging

    def mouseReleaseEvent(self, event):
        super().mouseReleaseEvent(event)
        self.scene_ref.update_lines() #update lines after releasing mouse
...
class CustomGraphicsScene(QGraphicsScene):
    ...
    def add_point(self, x, y):
        point = DraggablePoint(x, y, 5, len(self.points), self) #Pass self (scene) to the point
        self.addItem(point)
    ...
    def update_lines(self):
        ...
        if len(self.points) > 1: #Check if there are at least two points
            for i in range(len(self.points) - 1):
                line = QGraphicsLineItem(self.points[i].pos(), self.points[i + 1].pos()) #Use pos() for correct coordinates
                line.setPen(QColor("green"))
                self.addItem(line)
                self.lines.append(line)

        # Close the polygon by connecting the last point to the first
        if len(self.points) >= 3:
            line = QGraphicsLineItem(self.points[-1].pos(), self.points[0].pos()) #Use pos()
            line.setPen(QColor("green"))
            self.addItem(line)
            self.lines.append(line)
...
class CustomGraphicsView(QGraphicsView):
    def __init__(self, parent=None):
        ...
        self._pan_start = None  # For panning
...
class PinCalibrationApp(QMainWindow):
    def __init__(self):
        ...
        self.control_layout.addWidget(self.generate_drawing_button, 7, 0)  # Row 7, Column 0
        ...
        self.control_layout.addWidget(self.generate_step_button, 8, 0)  # Row 8, Column 0

        # Replace QLabel with QTextEdit for results
        self.result_text_edit = QTextEdit()
        self.result_text_edit.setReadOnly(True)  # Make it read-only
        self.control_layout.addWidget(self.result_text_edit, 9, 0, 1, 2)  # Row 9, Span 2 columns
        ...
        self.graphics_view.mousePressEvent = self.on_mouse_click
    ...
    def start_selecting_square(self):
        ...
        self.square_points = []
        self.graphics_scene.clear()
        self.display_image(self.image_rgb)  # Re-add the image
    ...
    def on_mouse_click(self, event):
        if self.selecting_square:
            ...
            x, y = pos.x(), pos.y()
            self.graphics_scene.add_point(x, y)
            self.update_result_text(f"Square corner {len(self.graphics_scene.points)} placed at ({x:.2f}, {y:.2f}).")
            if len(self.graphics_scene.points) == 4:
    ...
    def submit_square_points(self):
        ...
        src_points = np.array([(point.x(), point.y()) for point in self.graphics_scene.points], dtype="float32")
    ...
    def start_calibration(self):
        # Clear previous calibration points and line
    ...
    def submit_calibration_points(self):
        if len(self.graphics_scene.points) != 2:
        ...
        if ok:
            ...
            (x1, y1), (x2, y2) = [(point.x(), point.y()) for point in self.graphics_scene.points]
