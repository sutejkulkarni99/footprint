...
class PinCalibrationApp(QMainWindow):
    def __init__(self):
        ...
        self.submit_calibration_button.setStyleSheet("QPushButton { background-color: #009688; color: white; font-size: 16px; padding: 10px; }")
        ...
        self.reset_calibration_button.setEnabled(False)
        self.reset_calibration_button.setStyleSheet("QPushButton { background-color: #607D8B; color: white; font-size: 16px; padding: 10px; }")
        self.control_layout.addWidget(self.reset_calibration_button)
    ...
    def submit_calibration_points(self):
        ...
        dialog.setDoubleValue(1.0)  # Set the default value to 1.0
        ...
        if ok:
            ...
            self.reset_calibration_button.setEnabled(True)

    def reset_calibration(self):
        # Clear calibration points and visual markers
        self.calibration_points = []
        self.calibration_scatter.clear()

        # Reset calibration state
        self.pixel_to_mm_ratio = None
        self.result_label.setText("Calibration points reset. Click 'Calibrate' to reselect.")
        self.calibrate_button.setEnabled(True)
        self.submit_calibration_button.setEnabled(False)
        self.calculate_pitch_button.setEnabled(False)
        self.reset_calibration_button.setEnabled(False)
