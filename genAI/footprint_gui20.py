...
class PinCalibrationApp(QMainWindow):
    def __init__(self):
        ...
        # New variables for border and mounting holes
        self.border_size = 5.0  # Default border size in mm
        self.mounting_holes = []  # Stores mounting hole positions [(x1, y1), (x2, y2), ...]
        ...
        # Add "Add Border" and "Add Mounting Hole" buttons before file generation buttons
        self.add_border_button = QPushButton("Add Border")
        self.add_border_button.clicked.connect(self.add_border)
        self.add_border_button.setEnabled(False)  # Initially disabled
        self.add_border_button.setFixedSize(150, 40)
        self.control_layout.addWidget(self.add_border_button, 7, 0)  # Row 7, Column 0

        self.add_mounting_hole_button = QPushButton("Add Mounting Hole")
        self.add_mounting_hole_button.clicked.connect(self.add_mounting_hole)
        self.add_mounting_hole_button.setEnabled(False)  # Initially disabled
        self.add_mounting_hole_button.setFixedSize(150, 40)
        self.control_layout.addWidget(self.add_mounting_hole_button, 8, 0)  # Row 8, Column 0
        ...
        self.control_layout.addWidget(self.generate_drawing_button, 9, 0)  # Row 9, Column 0
        ...
        self.control_layout.addWidget(self.generate_step_button, 10, 0)  # Row 10, Column 0
    ...
    def on_mouse_click(self, event):
        ...
        elif self.selecting_mounting_hole:
            # Get the position relative to the view and map it to the scene
            pos = self.image_view.getView().mapSceneToView(event.pos())
            x, y = int(pos.x()), int(pos.y())

            # Add the mounting hole to the list
            self.mounting_holes.append((x, y))

            # Add visual marker for the mounting hole
            self.pin_scatter.addPoints([x], [y], pen='w', brush='w', symbol='+')

            self.update_result_text(f"Mounting hole {len(self.mounting_holes)} placed at ({x}, {y}).")

            # Submit the mounting hole if the limit is reached
            if len(self.mounting_holes) >= 4:
                self.submit_mounting_hole()
    ...
    def submit_row(self):
        if len(self.current_row) > 0:
            ...
            # Enable "Add Border" and "Add Mounting Hole" buttons
            self.add_border_button.setEnabled(True)
            self.add_mounting_hole_button.setEnabled(True)
    ...
    def generate_2d_drawing(self):
        ...
        if self.border_size > 0:
            ...
            border = Rectangle(
                (min(all_x) - self.border_size, min(all_y) - self.border_size),
                (max(all_x) - min(all_x)) + 2 * self.border_size,
                (max(all_y) - min(all_y)) + 2 * self.border_size,
                linewidth=1, edgecolor='k', facecolor='none', linestyle='--'
            )
            ax.add_patch(border)

        # Add mounting holes to the plot
        for hole in self.mounting_holes:
            x, y = hole
            circle = Circle((x, y), 1.0, edgecolor='k', facecolor='none', linestyle='--')  # 1.0mm diameter
            ax.add_patch(circle)
            ax.text(x, y + 2, 'Mounting Hole', fontsize=8, ha='center', va='bottom', color='k')
    ...
    def add_border(self):
        """
        Add a border around the pin layout.
        """
        # Open a dialog to input the border size
        dialog = QInputDialog(self)
        dialog.setWindowTitle("Add Border")
        dialog.setLabelText("Enter the border size (in mm):")
        dialog.setInputMode(QInputDialog.DoubleInput)
        dialog.setDoubleMinimum(1.0)  # Minimum border size
        dialog.setDoubleValue(self.border_size)  # Default value

        # Show the dialog and get the result
        ok = dialog.exec()
        if ok:
            self.border_size = dialog.doubleValue()
            self.update_result_text(f"Border size set to {self.border_size} mm.")

    def add_mounting_hole(self):
        """
        Add a mounting hole to the design.
        """
        # Enable mounting hole selection
        self.selecting_mounting_hole = True
        self.selecting_square = False
        self.selecting_calibration = False
        self.selecting_pins = False

        # Change button text
        self.add_mounting_hole_button.setText("Submit Mounting Hole")
        self.add_mounting_hole_button.clicked.disconnect()
        self.add_mounting_hole_button.clicked.connect(self.submit_mounting_hole)
        ...
        self.update_result_text("Click on the image to place a mounting hole.")

    def submit_mounting_hole(self):
        """
        Submit the mounting hole position.
        """
        if len(self.mounting_holes) >= 4:  # Limit the number of mounting holes
            QMessageBox.warning(self, "Warning", "Maximum of 4 mounting holes allowed.")
            return

        # Disable mounting hole selection
        self.selecting_mounting_hole = False
        self.image_view.scene.sigMouseClicked.disconnect(self.on_mouse_click)

        # Change button text back
        self.add_mounting_hole_button.setText("Add Mounting Hole")
        self.add_mounting_hole_button.clicked.disconnect()
        self.add_mounting_hole_button.clicked.connect(self.add_mounting_hole)

        self.update_result_text(f"Mounting hole {len(self.mounting_holes) + 1} placed.")
    ...
    def reset(self):
        ...
        self.generate_drawing_button.setEnabled(False)
        self.generate_step_button.setEnabled(False)
        self.add_border_button.setEnabled(False)
        self.add_mounting_hole_button.setEnabled(False)
