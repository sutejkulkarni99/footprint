# -*- coding: utf-8 -*-
"""
Created on Fri Jan 24 14:47:49 2025
...
def approximate_polygon_to_rectangle_or_square(polygon_points):
    """
    Approximate the selected polygon to a standard rectangle or square.

    Parameters:
    - polygon_points: List of polygon points (x, y).

    Returns:
    - List of approximated rectangle or square points (x, y).
    """
    ...
    # Define the approximated rectangle or square
    approximated_points = [
        (min_x, min_y),
        (max_x, min_y),
        (max_x, max_y),
...
def create_ipc7351_footprint(filename, footprint_name, pin_rows, pin_diameter, tolerance=0.2, annular_ring=0.15, polygon_points=None):
    """
    ...
    - pin_rows: List of rows of pins. Each row is a list of pin coordinates (in mm).
    - pin_diameter: Diameter of the pins (in mm).
    - tolerance: Drill tolerance (in mm).
    - annular_ring: Annular ring width (in mm).
    - polygon_points: List of polygon points for the border (optional).
    """
    drill_size = pin_diameter + tolerance
    pad_size = drill_size + 2 * annular_ring
    pad_size = max(pad_size, 1.5)  # Ensure minimum pad size

    # Calculate the total width and height of the footprint based on the maximum row length
    total_width = max(pin[0] for row in pin_rows for pin in row) - min(pin[0] for row in pin_rows for pin in row) + pad_size
    total_height = max(pin[1] for row in pin_rows for pin in row) - min(pin[1] for row in pin_rows for pin in row) + pad_size

    # Define the courtyard and silkscreen dimensions
    courtyard_clearance = 0.25  # Standard clearance for courtyard
    courtyard_width = total_width + 2 * courtyard_clearance
    courtyard_height = total_height + 2 * courtyard_clearance
    ...
    content += f"  (fp_text reference REF** (at 0 {-courtyard_height/2 - 2}) (layer F.SilkS)\n"
    content += "    (effects (font (size 1 1) (thickness 0.15))))\n"
    content += f"  (fp_text value {footprint_name} (at 0 {courtyard_height/2 + 2}) (layer F.Fab)\n"
    content += "    (effects (font (size 1 1) (thickness 0.15))))\n"

    # Add pads for all rows
    for row_idx, row in enumerate(pin_rows):
        for pin_idx, (x, y) in enumerate(row):
            # Calculate the position of each pin
            content += f"  (pad {row_idx * len(row) + pin_idx + 1} thru_hole circle (at {x} {y}) (size {pad_size} {pad_size}) (drill {drill_size}) (layers *.Cu *.Mask F.SilkS))\n"
    ...
    # Add polygon border if provided
    if polygon_points:
        for i in range(len(polygon_points)):
            x1, y1 = polygon_points[i]
            x2, y2 = polygon_points[(i + 1) % len(polygon_points)]
            content += f"  (fp_line (start {x1} {y1}) (end {x2} {y2}) (layer F.SilkS) (width 0.15))\n"
...
class PinCalibrationApp(QMainWindow):
    ...
    def generate_footprint(self):
        """
        Generate an IPC-7351 compliant footprint based on the selected pins, pitch, and border.
        """
        if len(self.pin_rows) < 1:
        ...
        # Calculate the pitch in mm for each row
        pitches = []
        for row in self.pin_rows:
            if len(row) > 1:
                pitch_pixels = np.sqrt((row[1][0] - row[0][0])**2 + (row[1][1] - row[0][1])**2)
                pitch_mm = pitch_pixels / self.pixel_to_mm_ratio
                # Approximate the pitch to the nearest standard pitch size
                nearest_pitch = min(self.standard_pitch_sizes, key=lambda x: abs(x - pitch_mm))
                pitches.append(nearest_pitch)
            else:
                pitches.append(0)
    
        # Define the pin diameter (default: 1.0 mm)
        pin_diameter = 1.0  # You can make this configurable if needed
    
        # Approximate the selected polygon to a standard rectangle or square
        if len(self.lasso_points) > 2:
            # Transform border points using homography and pixel-to-mm ratio
            transformed_border_points = self.transform_points(self.lasso_points)
            approximated_polygon = approximate_polygon_to_rectangle_or_square(transformed_border_points)
        else:
            approximated_polygon = None
    
        # Realign pins and rows
        aligned_pin_rows = []
        y_offset = 0
        for row_idx, row in enumerate(self.pin_rows):
            aligned_row = []
            x_offset = 0
            for pin_idx in range(len(row)):
                if pin_idx == 0:
                    x_offset = 0
                else:
                    x_offset += pitches[row_idx]
                aligned_row.append((x_offset, y_offset))
            aligned_pin_rows.append(aligned_row)
            y_offset += min(self.standard_row_separations, key=lambda x: abs(x - y_offset))
        ...
        try:
            create_ipc7351_footprint(
                ...
                pin_rows=aligned_pin_rows,  # Pass aligned pin rows
                pin_diameter=pin_diameter,  # Pin diameter in mm
                polygon_points=approximated_polygon  # Approximated polygon points
            )
