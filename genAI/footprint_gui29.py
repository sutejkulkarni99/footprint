...
def create_ipc7351_footprint(
    ...
    for i in range(num_pins):
        ...
        y = 0
        content += f"  (pad {i+1} {pad_type} {pad_shape} (at {x} {y}) (size {pad_width} {pad_height}) (layers *.Cu *.Mask F.SilkS))\n"
...
class PinCalibrationApp(QMainWindow):
    ...
    def calculate_border_width(self):
        ...
        if len(self.pin_rows) < 1:
            return 0.0  # Default value if no pins are selected
        ...
        return width_mm + 5.0  # Add 5mm padding
    ...
    def calculate_border_height(self):
        ...
        return height_mm + 5.0  # Add 5mm padding

    def calculate_pitch(self):
        """
        Calculate the pitch between pins in millimeters.
        """
        if len(self.pin_rows) < 1:
            return 0.0  # Default value if no pins are selected

        # Calculate the pitch based on the distance between the first two pins in the first row
        x1, y1 = self.pin_rows[0][0]
        x2, y2 = self.pin_rows[0][1]

        # Convert pixel distance to millimeters using the calibration ratio
        pitch_pixels = np.sqrt((x2 - x1)**2 + (y2 - y1)**2)
    ...
    def generate_footprint(self):
        ...
        try:
            create_ipc7351_footprint(
                ...
                pitch=self.calculate_pitch(),  # Pitch in mm
                pin_diameter=1.0,  # Default pin diameter
                border_width=self.calculate_border_width(),  # Border width in mm
                border_height=self.calculate_border_height(),  # Border height in mm
