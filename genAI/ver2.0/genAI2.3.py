...
def create_output_file(extracted_data, output_file, output_format):
    """
    Create a new file (Excel, CSV, or XML) with the extracted data in the desired format.
...
def select_input_file():
    """
    Open a file dialog to select an input Excel file.
    """
...
def select_output_file(output_format):
    """
    Open a file dialog to select a location for saving the output file based on the selected format.
    """
    filetypes = {
    ...
    file_path = filedialog.asksaveasfilename(defaultextension=f".{output_format}", filetypes=filetypes[output_format])
    return file_path

def run_extraction(input_file, output_file, output_format, progress_label):
    """
    Run the extraction and save process, showing progress in the GUI.
    """
    try:
        ...
        # Show status of output file creation
        progress_label.config(text="Creating output file... Please wait.")
        if not output_file:
            raise ValueError("Output file path is invalid. Please specify a valid file path.")
...
def create_gui():
    """
    Create the GUI window with buttons and status labels.
    """
    window = tk.Tk()
    window.title("AI Data Extraction from Excel")
    ...
    format_combobox.set('excel')  # Default to Excel format
    ...
    start_button = tk.Button(window, text="Start Processing", 
                             command=lambda: run_extraction(input_file_entry.get(), output_file_entry.get(), format_combobox.get(), progress_label))
    start_button.pack(pady=20)

    # Open the file dialogs when the user selects the respective entry fields
